...
class ConvTransformerBackbone(nn.Module):
    ...
    def forward(self, vid, txt, mask):
        ...
        lv_vid = tuple()
        lv_masks = tuple()
        lv_self_attn_txt = tuple() # only self-attention for text
        # 1x resolution
        lv_vid += (vid, )
        lv_masks += (mask, )
        lv_self_attn_txt += (txt, )        
        

        # main branch with downsampling
        for idx in range(len(self.branch)):
            vid, mask, txt, self_attn_txt = self.branch[idx](vid, txt, mask)
            lv_vid += (vid, )
            lv_masks += (mask, )
            lv_self_attn_txt += (self_attn_txt, )
        
        return lv_vid, lv_masks, lv_self_attn_txt